...
opt,data = ncpop([-f], [x;y], 1, TS=false, partition=2, constraint="unipotent")
```
...
opt,data = ncpop([-f], [x;y], 3, TS=false, normality=true, partition=3, constraint="projection")
```
...
λ = @variable(model)
p = add_poly!(model, y, 2d, constraint="unipotent")
q = add_poly!(model, y, 2d, constraint="unipotent")
