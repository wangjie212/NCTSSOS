...
function nctssos_first(
    f::Polynomial{false,T} where {T<:Number},
    ...
    if reducebasis == true && obj == "eigen" && constraint === nothing
        ...
        if flag == 1
            ...
            if monosquare == true
                if obj == "trace"
                    append!(
                        ksupp, [_cyclic_canon([item[end:-1:1]; item]) for item in basis]
                    )
                else
                    append!(ksupp, [[item[end:-1:1]; item] for item in basis])
                end
            ...
            blocks, cl, blocksize = get_blocks(
                ksupp, basis; TS=TS, QUIET=QUIET, merge=merge, md=md, obj=obj
            )
        end
    end
    if QUIET == false
...
function get_blocks(
    ...
    if TS == false
        ...
        blocks = [[i for i in 1:length(basis)]]
        cl = 1
    else
...
function solvesdp(
    ...
    k = 1
    for i in 1:cl, j in 1:blocksize[i], r in j:blocksize[i]
    ...
    if solve == true
        ...
        for i in 1:cl
            ...
            if bs == 1
                ...
                for j in 1:blocksize[i], r in j:blocksize[i]
                    @inbounds bi = [basis[blocks[i][j]][end:-1:1]; basis[blocks[i][r]]]
                    bi = reduce!(bi; obj=obj, partition=partition, constraint=constraint)
                    Locb = bfind(ksupp, lksupp, bi)
                    if j == r
