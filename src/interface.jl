...
function CliqueTrees.cliquetree(graph;alg::NoElimination)
    # CliqueTree
    # supernodetree
    # count # looks like all vertices
    # sep # a bipartitegraph
    # nov     # what is this? number of vertices
    # ptr     # what is this?
    # tgt

    # NOTE: I am not sure if this is the correct way to construct a CliqueTree of a complete graph
    return cliquetree(complete_graph(length(vtcs)), BFS())
end
...
function cs_nctssos_higher(pop::PolynomialOptimizationProblem{C,T},prev_res::PolyOptResult,solver_config::SolverConfig)
