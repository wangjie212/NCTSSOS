...
@testset "case 1" begin
    ...
    opt,data = ncpop([f], x, 2, newton=true, CS=false, TS="MD", obj="eigen", QUIET=true)
...
@testset "case 2" begin
    ...
    opt,data = ncpop(pop, x, 2, numeq=1, TS="MD", obj="eigen", QUIET=true)
    @test isapprox(opt, -1, atol=1e-6)

    opt,data = ncpop(data, TS="MD", QUIET=true)
    @test isapprox(opt, -1, atol=1e-6)

    opt,data = ncpop(pop, x, 2, numeq=1, TS="MD", obj="trace", QUIET=true)
    @test isapprox(opt, -1, atol=1e-6)
...
@testset "case 3" begin 
    ...
    opt,data = ncpop([f], x, 3, TS="block", obj="trace", QUIET=true)
    @test isapprox(opt, 0.0, atol=1e-6)

    opt,data = ncpop(data, TS="block", QUIET=true)
    ...
    opt,data = ncpop(pop, x, 3, TS="block", obj="eigen", QUIET=true)
    @test isapprox(opt, 3.4055933, atol=1e-6)

    opt,data = ncpop(pop, x, 3, TS="block", obj="trace", QUIET=true)
