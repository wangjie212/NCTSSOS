...
@testset "poly info" begin
    ...
    expected_sym_coe = [2, 142, -2, 18, -54, 1, -2, 3, -2, 6, -1]
    ...
    expected_cyc_supp = map(
        x -> minimum(circshift(x, i) for i in 0:(length(x) - 1)),
        [
            [1, 2, 2, 1],
            [3, 2, 2, 3],
            [1, 2, 1],
    ...
    expected_cyc_coe = [7, 142, -2, 18, -54, 1, -2, 3, -2, 6, 10]
...
@testset "newton_ncbasis" begin
    ...
    @test sort(cycbasis) ==
        sort([Int[], [1], [2], [3], [1, 2], [1, 3], [2, 1], [2, 3], [3, 1], [3, 2]])
...
@testset "get_blocks" begin
    ...
    blocks, cl, blocksize = NCTSSOS.get_blocks(
        supp,
        basis;
        TS="MD",
        QUIET=true,
...
@testset "case 2" begin
    ...
    opt, data = nctssos_first(
        pop, x, 2; numeq=1, TS="MD", obj="eigen", QUIET=true, solver="COSMO"
    )
    ...
    opt, data = nctssos_first(
        pop, x, 2; numeq=1, TS="MD", obj="trace", QUIET=true, solver="COSMO"
    )
    @test isapprox(opt, -1, atol=1e-6)
...
@testset "case 3" begin
    ...
    opt, data = cs_nctssos_first(
        [f], x, 3; TS="MD", obj="trace", QUIET=true, solver="COSMO"
    )
