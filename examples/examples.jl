...
f =
    3 + x[1]^2 + 2x[1]^3 + 2x[1]^4 + x[1]^6 - 4x[1]^4 * x[2] +
    x[1]^4 * x[2]^2 +
    4x[1]^3 * x[2] +
    2x[1]^3 * x[2]^2 - 2x[1]^3 * x[2]^3 + 2x[1]^2 * x[2] - x[1]^2 * x[2]^2 +
    8x[1] * x[2] * x[1] * x[2] +
    2x[1]^2 * x[2]^3 - 4x[1] * x[2] +
    4x[1] * x[2]^2 +
    6x[1] * x[2]^4 - 2x[2] + x[2]^2 - 4x[2]^3 +
    2x[2]^4 +
    2x[2]^6
...
f =
    x[2] * x[1] + x[2] * x[1]^2 * x[2] - 2x[2] * x[1]^2 + x[2] * x[1] * x[2] * x[1] -
    2x[2] * x[1] * x[2] +
    x[1] * x[2] +
    x[1] * x[2] * x[1] * x[2] - 2x[1]^2 * x[2] + x[2] * x[1]^2 * x[2] - 2x[2] * x[1] * x[2]
...
@ncpolyvar x[1:6]
f =
    x[1] * (x[4] + x[5] + x[6]) + x[2] * (x[4] + x[5] - x[6]) + x[3] * (x[4] - x[5]) -
    x[1] - 2x[4] - x[5]
@time opt, data = cs_nctssos_first(
    -f, x, 2; partition=3, constraint="projection", TS=false, QUIET=true, obj="eigen"
)

@time opt, data = nctssos_first(
    [-f],
    x,
    5;
    add_soc=true,
