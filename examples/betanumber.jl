...
@time opt,data = stateopt_first(supp, coe, n, d, numeq=Int(n*(n-1)/2), QUIET=true, TS="block", constraint="unipotent")
...
sort!(edges)
n = 5
supp = Vector{Vector{Vector{UInt16}}}(undef, Int(n*(n-1)/2)+1)
coe = Vector{Vector{Float64}}(undef, Int(n*(n-1)/2)+1)
supp[1] = [[i] for i = 1:n]
coe[1] = -ones(n)
k = 2
eq = zeros(Int, Int(n*(n-1)/2))
for i = 1:n, j = i+1:n
    supp[k] = [[i;j], [j;i]]
    if NCTSSOS.bfind(edges, length(edges), [i;j]) === nothing
        coe[k] = [1; -1]
...
d = 2 # relaxation order
@time opt,data = nctssos_first(supp, coe, n, d, numeq=Int(n*(n-1)/2), eq_constraint_type=eq, QUIET=true, TS=false, constraint="unipotent")
