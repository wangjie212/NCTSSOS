...
for i in 1:l
    ...
    supp[i + 1] = [
        [UInt16[]]
        [UInt16[j; j] for j in ((b - 5) * i - (b - 6)):((b - 5) * i + 5)]
    ]
...
supp = Vector{UInt16}[
    [1; 4], [1; 5], [1; 6], [2; 4], [2; 5], [2; 6], [3; 4], [3; 5], [1], [4], [5]
]
coe = -[1, 1, 1, 1, 1, -1, 1, -1, -1, -2, -1]
n = 6
supp = Vector{UInt16}[
    [1; 4], [1; 5], [1; 6], [2; 4], [2; 5], [2; 6], [3; 4], [3; 5], [1], [2], [4], [5], []
]
coe =
    -[
        1 / 4,
...
opt, data = nctssos_first(
    [supp], [coe], n, 3; TS=false, partition=3, constraint="projection"
)
